"""
clone https://github.com/automan000/Convolutional_LSTM_PyTorch/blob/master/convolution_lstm.py
"""
import torch
...
class ConvLSTM(nn.Module):
    ...
    def __init__(self, input_channels, hidden_channels, kernel_size, step=1, effective_step=[1], device=0, window=3):
        ...
        for i in range(self.num_layers):
            name = 'cell{}'.format(i)
            cell = ConvLSTMCell(self.input_channels[i], self.hidden_channels[i], self.kernel_size, self.device)
            setattr(self, name, cell)
            self._all_layers.append(cell)

        self.forecast = nn.Sequential(
            nn.Conv2d(self.hidden_channels[-1]*self.window, 1, kernel_size=(1, 1), stride=1, padding=0),
        )

    def forecasting(self, output):
        output = output.view(1, output.shape[0]*output.shape[1], 64, 64)
        output = self.forecast(output)
        return output
    ...
    def forward(self, input):
        ...
        outputs = self.forecasting(outputs[0])
