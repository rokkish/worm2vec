...
import pandas as pd
import get_logger
logger = get_logger.get_logger(name='sprite')

def create_sprite_image(images):
    """create image for tensorboard projector.

    Args:
        images (tensor): (batch size, H*W==28*28) 
    """

    # size of sprite image (28 pixel)
    img_h = int(np.ceil(np.sqrt(images.shape[1])))
    img_w = img_h
...
def save_sprite_image(sprite_image, path):
    print("sprite :", sprite_image.min(), sprite_image.max())
    sprite_image = 1. - sprite_image
    print("reverse:", sprite_image.min(), sprite_image.max())
