"""predict mean of 2d-shapes like CBOW
"""
import tensorflow as tf
from models.autoencoder import AutoEncoder


class TwoDshapeCBOW_model(object):
    def __init__(self, x_previous, x_next, x_now, input_dim, multiply_dim, share_enc_trainable):
        ...
        self.W_enc_context = tf.get_variable(
            'w_enc_context',
            shape=[self.context_enc_dim, self.latent_dim],
            dtype=tf.float32,
...
def nn_cbow_loss(context, target):
    ...
    with tf.name_scope('distance_loss'):
        ...
        distance = cosine_similarity(context, target)
